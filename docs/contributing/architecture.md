...
| Runtime adapters | Start QEMU, Firecracker, libkrun, or Apple Virtualization.framework and handle backend-specific work. | [`src/smolvm/runtime/`](../../src/smolvm/runtime) |
| Guest communication | Chooses and implements SSH or vsock control. | [`src/smolvm/comm/`](../../src/smolvm/comm) |
| Images | Finds, builds, caches, and prepares boot images, including local-only macOS bundles. | [`src/smolvm/images/`](../../src/smolvm/images), [`src/smolvm/macos/`](../../src/smolvm/macos) |
...
4. `SmolVM` waits for the selected control channel, then performs commands or file operations. A macOS desktop instead becomes ready when its loopback display is available; SSH may follow later.
